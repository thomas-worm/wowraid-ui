...
export class RaidAttendeeListComponent implements OnInit {
  ...
  private filterClass(attendees: EventAttendee[], characterClass: String): EventAttendee[] {
    return attendees.filter(attendee => attendee.character_class.toLocaleLowerCase() == characterClass.toLocaleLowerCase());
  }

  private filterRole(attendees: EventAttendee[], role: String): EventAttendee[] {
    return attendees.filter(attendee => attendee.roles.filter(roleEntry => roleEntry.toLocaleLowerCase() == role.toLocaleLowerCase()).length > 0);
  }
